...
ann = ANN(epochs=100, batch=10, n_in=n_in, n_out=n_out)
...
ann.set_test(test_X, test_y)

predict = annModel.predict(test_X)

temps = np.concatenate((test_X, predict), axis=1)
reversed = sc.inverse_transform(temps)
predict_changes = reversed[:, -n_out:]

plt.plot(test_y[0], label='real_change')
plt.plot(predict_changes[0], label='predict_change')
plt.legend()
plt.show()
...
cost, pos = optimizer.optimize(objective_function, iters=100, verbose=1)

pso_model = ann.model_pso(pos)
pso_predict = pso_model.predict(test_X)
pso_temps = np.concatenate((test_X, pso_predict), axis=1)
pso_reversed = sc.inverse_transform(pso_temps)
pso_predict_changes = pso_reversed[:, -n_out:]

plt.plot(test_y[0], label='real_change')
plt.plot(predict_changes[0], label='ann_predict_change')
plt.plot(pso_predict_changes[0], label='pso_predict_change')
plt.legend()
plt.show()
