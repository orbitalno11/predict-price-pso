...
class Indicator:
    ...
    def EMA(self,EMA_n_day:int = None) -> DataFrame:
        ...
        for index in self.__dataset.index: 
            if index > (self.__ema_n_day-1):
                ...
                EMA = previous_EMA + (2/self.__ema_n_day+1)*(self.__dataset.iloc[[index]]['Close'].values[0]-previous_EMA)
    ...
    def MACD(self, fast_period: int = 12, slow_period: int = 26, signal_period: int = 9) -> DataFrame:
        ...
        first_ema = self.__dataset.loc[slow_period-1:(slow_period-1)+(signal_period-1)]['MACD'].sum()/signal_period
        ...
        for index in self.__dataset.index: 
            if index > (slow_period-1)+(signal_period-1):
                ...
                EMA = previous_EMA + (2/signal_period+1)*(self.__dataset.iloc[[index]]['MACD'].values[0]-previous_EMA)
